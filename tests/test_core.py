...
def test__set_zoo_biomass():
    """_set_zoo_biomass should set the zooplankton biomass, but nothing else"""
...
def test__set_zoo_mortality():
    """_set_zoo_mortality should set the zooplankton mortality"""

    data = np.random.rand(n_zoo, NX)
    F._set_zoo_mortality(data)

    # make sure it worked
    assert (F.zoo_mortality.data == data).all()

    # put it back
    F._set_zoo_mortality(data * 0.0)


def test__set_zoo_mortality_wrong_shape():
    """_set_zoo_mortality should raise assertion error if shape is wrong"""
    data = np.random.rand(42, NX * 2)
    with pytest.raises(AssertionError):
        F._set_zoo_mortality(data)


def test__set_fish_biomass():
    """_set_fish_biomass should set the fish biomass, but nothing else"""
...
def test_set_benthic_biomass():
    ...
    F._set_benthic_prey_biomass(data)
